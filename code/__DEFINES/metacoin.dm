...
#define METACOIN_SURVIVE_REWARD          (GLOB.player_list.len * 100)
/// Rewarded when you don't survive the round, but stick around till the end
#define METACOIN_NOTSURVIVE_REWARD       (max(GLOB.player_list.len * 10, 100))
/// Rewarded when you are alive and active for 10 minutes
#define METACOIN_TENMINUTELIVING_REWARD  (GLOB.player_list.len * 5)
